-- 매출 원가 — 서브 작가에게도 출장비가 나간다 (대표 2026-09-16 «서브도 3만원 나감»)
--
-- 20260824a 부터 서브 원가를 15만원 고정으로 잡고 출장비를 안 얹고 있었다.
-- 그래서 2인 촬영 + 경기 출장인 예식마다 **순이익이 3만원씩 실제보다 높게** 잡혀 있었다.
-- 2026-09-16 실측 3건(지난 2 · 앞으로 1) — 모두 9만원. 작아서 눈에 안 띄었다.
--
-- ⚠ 함수 전문은 **DB 에서 받아 한 군데만 바꿔** 다시 만든 것이다
--   (.backups/tests/pgrun/_subtravel_patch.mjs). 손으로 옮겨 적지 않았다.
-- ⚠ 작가비 카드 쪽은 private.pay_overdue 에 있다(20260916c). **작가비를 바꿀 때는 두 곳을 같이 고친다.**

CREATE OR REPLACE FUNCTION public.admin_sales(p_span text DEFAULT 'now'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  liv as (
    select b.id, b.wedding_date, b.total_price, b.assignee_id, b.line_items,
           ...
           + case when exists (
               ...
               then c_sub + case when b.travel_fee then c_travel else 0 end else 0 end as cost
