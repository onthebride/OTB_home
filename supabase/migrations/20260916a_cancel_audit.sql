...
--   · 배정 이력 화면 맨 위 「작가 배정됨」 카드도 하나 준다
--     (admin_assignment_audit 의 upcoming_assigned
--      = status <> '취소' and wedding_date >= current_date and assignee_id is not null)
-- 숫자는 줄었는데 이력에는 아무것도 없으니 **왜 줄었는지 알 길이 없었다.**
...
--
-- ⚠ 폰 경보(private.assignment_health_check)는 취소로는 **안 울린다. 그게 맞다.**
--   2026-08-30 에 헛경보를 고치면서 경보 기준을 assigned_all
--   (= 작가가 박혀 있는 예약 수, 취소·지난 것 포함) 로 바꿨다. 취소해도 assignee_id 는
--   남으므로 그 수는 안 준다 — 취소를 사고로 오해하지 않으려고 일부러 그렇게 뒀다.
--   (2026-09-16 에 8/19 원본만 보고 «곧 경보가 울린다» 고 잘못 적었다가 바로잡았다.
--    이 함수는 20260830a·20260831f 에서 두 번 갈아끼워졌다 — 원본만 보지 말 것)
