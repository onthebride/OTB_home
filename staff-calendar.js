...
function feeNow(d) {
  ...
  /* ⚠ 안 정하셨을 때 신부님이 내시는 **확정값**은 있다. 그 돈이 누구 몫인지는
     대표께 여쭤둔 것이라, 여기서는 **누구 몫이라고 말하지 않는다.**
     모르는 것을 작가님께 단정해 적으면 그게 곧 약속이 된다 */
  const none = `지금은 신부님이 <b>${wonFmt(pin)}원</b>에 지정하실 수 있어요.`;
