...
function feeNow(d) {
  ...
  /* 금액을 안 정하셨으면 확정값에 팔리고 **그 돈은 회사 몫**이다
     (대표 2026-09-16 «금액 없으면 지정값 3만원» → «그건 우리가 가져감»).
     ⚠ 정하시면 **전액** 받으신다는 것을 같이 적는다 — 대표 «작가들 동기부여 최대한».
       정할 까닭이 바로 이 한 줄에 있다. 길게 쓰지는 않는다 («문구 너무 친절하네») */
  const none = `신부님은 <b>${wonFmt(pin)}원</b>에 지정하실 수 있고, 그건 <b>회사 몫</b>이에요.`
    + ` 금액을 정하시면 <b>전액</b> 받으세요.`;
  ...
  box.innerHTML = `
    <div class="sc-set">
      ...
      <section class="sc-set-row">
        ...
        <!-- ⚠ 설명 문단을 통째로 뺐다 (대표 2026-09-16 «이거 문구 너무 친절하네 그냥 빼자»).
             ⚠ 다시 넣고 싶어지면 참고: 여기 있던 것은 「지정하실 때 더해지는 금액 / 6만원부터 /
               그중 3만원은 회사 몫 / 안 받으시려면 0」 넉 줄이었다.
               그 내용은 다른 데 남아 있다 — 최소는 입력칸 예시(placeholder)와 저장할 때 알림에,
               회사 몫과 받으실 돈은 아래 sc-set-now(feeNow)에. **지우면서 사라진 사실은 없다.** -->
        ${reqList(d)}
        ${d.can_fee ? `
        ${capBox(d)}
        <div class="sc-fee">
