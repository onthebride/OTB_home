...
/* 회사 수수료 — **지금은 안 뗀다** (대표 2026-09-16 «지정하는건 우리가 안가져가는걸로하자»).
   그날 낮에 «50% 최대 3만원» 으로 정하셨다가 한 시간 만에 뒤집으셨다.
   ⚠ 셈하는 길은 살려 둔다. 서버가 cut_pct·cut_max 를 0 으로 주고 있을 뿐이라,
     다시 떼기로 하시면 서버 두 줄만 고치면 화면이 그대로 따라온다.
   ⚠ 숫자는 서버에서 받아 쓴다. 여기 50·30000 을 적어두면 서버와 어긋난다 —
     **없으면 0**(안 뗌)이 기본이다 */
const cutOf = (d, fee) => Math.min(
  Math.floor((Number(fee) || 0) * (Number(d && d.cut_pct) || 0) / 100),
  Number(d && d.cut_max) || 0);
...
/* 지금 얼마로 되어 있나 + 내가 받는 몫
   ...
   ⚠ 0 의 뜻이 바뀌었다 — 「안 팔린다」가 아니라 **「그래도 팔린다」**
     (대표 «금액 없으면 지정값 3만원»). 그 3만원도 작가님이 받으신다.

   ⚠ 여기 있던 설명 문단(「지정하실 때 더해지는 금액 / 6만원부터 / 안 받으시려면 0」)은
     통째로 뺐다 (대표 «이거 문구 너무 친절하네 그냥 빼자»). 그 사실들은 다른 데 남아 있다 —
     최소는 입력칸 예시(placeholder)와 저장할 때 알림에, 받으실 돈은 아래 sc-set-now 에.
     ⚠⚠ 이런 메모를 **template literal 안에 <!-- --> 로 적지 말 것.**
       HTML 주석은 innerHTML 에 그대로 실려 나간다 — 2026-09-16 에 그렇게 적었다가
       시험이 잡았다. 사람에게 남길 말은 여기(JS 주석)에 적는다 */
function feeNow(d) {
  const pin = Number(d.pin_default) || 30000;
  /* 금액을 안 정하셨어도 확정값에 팔린다 (대표 2026-09-16 «금액 없으면 지정값 3만원»).
     ⚠ **그 3만원도 작가님이 받으신다** — 대표가 같은 날 두 번 바꾸셨다:
       «그건 우리가 가져감» → «지정비용 3만원도 작가들 주자».
       그래서 지정에서 회사가 가져가는 것은 **아무것도 없다**.
     ⚠ 「회사 몫이에요 / 정하시면 전액 받으세요」 두 마디는 뺐다 (대표 «그 문구 빼주고~»).
  ...
  return `지금 <b>${wonFmt(d.pick_fee)}원</b> — 신부님이 지정하시면 그만큼 더 내십니다.`
    ...
    // 안 뗄 때는 이 줄을 아예 안 그린다 — 「회사 몫 0원」 이라고 적으면 떼는 줄 아신다
    + (cut > 0 ? ` <span class="sc-fee-cut">(회사 몫 ${wonFmt(cut)}원)</span>` : '');
}

/* 「설정」 칸을 그린다.
   ⚠ 「지정 촬영은 빠른 시일 내에 도입될 예정입니다」 — 대표 요청 2026-08-29.
     아직 지정 예약을 안 받고 있어서, 금액만 적어두고 «왜 지정이 안 들어오지» 하고
     기다리시지 않게 먼저 알린다.
...
/* 미리 알림 칸에 대해 남길 말 (2026-09-25)
   ⚠⚠ 이 말을 **template literal 안에 <!-- --> 로 적지 말 것.**
     HTML 주석은 innerHTML 에 그대로 실려 화면으로 나간다 — 2026-09-16 에 그랬고
     오늘 또 그랬다. 사람에게 남길 말은 여기(JS 주석)에 적는다.
   · 「우리 예식과 …」 로 시작하던 한 줄은 뺐다 (대표 «이건 빼자»).
     빼도 **하는 일은 그대로다** — 작가님이 적어두신 일정도 알려드린다. 글만 줄인 것이다.
     ⚠ 그 문장을 주석에도 그대로 옮겨 적지 않는다. 시험이 「없어졌나」를 글자로 본다
   · 고르는 칸은 넷이다 (대표 «몇일 전 날짜를 더 디테일하게 / 2일 전 하나 더 넣으면 되려나») */
...
function renderSet() {
  ...
  box.innerHTML = `
    <div class="sc-set">
      ...
      <section class="sc-set-row">
        <h3>미리 알림</h3>
        <p class="sc-set-d">다가오는 일정을 <b>미리 폰으로</b> 알려드려요.<br />
          원하시는 것만 켜두세요. 아무것도 안 켜시면 안 보내드립니다.</p>
        <div class="sc-rm" id="setRmDays">
          ${[[3, '3일 전'], [2, '2일 전'], [1, '하루 전'], [0, '당일']].map(([v, t]) => `
