...
function renderDashboard() {
  ...
  /* ③ 배정 확인 — 배정 알림을 읽었나 (대표 2026-08-31
...
/* 작가에게 보낼 안내문 (대표 2026-08-31 «작가 안내문을 만들자» → «넘 긴데 간결하게 가자»).
   링크는 작가마다 다르다 — 작가 관리에서 📋 를 누르면 그 사람 것이 만들어진다.

   ⚠ 짧게 쓴다. 카톡에서 읽는 글이라 길면 안 읽고 넘기신다.
     자세한 것은 캘린더 [안내] 칸에 다 있다 — 여기서는 **거기로 보내기만** 한다.
     4시간 규칙·연락처 2주·석 달 자동 멈춤 같은 것을 여기 또 적지 않는다.
   ⚠ 그래도 이 넷은 뺄 수 없다.
...
function staffGuideMsg() {
  return [
    ...
    '',
    /* ⚠ 「불가 여부를 수요일까지」는 뺐다 (대표 2026-08-31 «이번 일회성이니까 빼»).
       이 글은 새 작가 들어오실 때도 다시 쓰는 틀이라, 이번 한 번만 필요한 말을 넣어두면
       다음에 그대로 나간다. 이번에 보내실 때만 대표가 한 줄 붙이신다 */
    '자세한 사용법은 [안내] 칸에 있습니다.',
...
function staffCalMsg(id) {
  ...
  const ch = STAFF_CHANNEL
    ? ['', '카카오톡 채널 [' + STAFF_CHANNEL_NAME + ']', STAFF_CHANNEL]
    : [];
  return [
...
/* 작가 미확인 — 배정 확인 / 월요일 체크 / 설문 확인
   (2026-08-28 둘로 나눔 · 2026-08-31 「배정 확인」이 앞에 붙음 · 2026-09-17 이름을 그렇게 고침)
...
/* ⚠ booking_cancelled·booking_restored 는 배정 칸이 아니라 **예식 상태**가 바뀐 줄이다
     ...
     예식이 취소되면 배정 칸은 그대로인데 위 「작가 배정됨」 카드는 하나 준다
     (그 카드는 취소를 뺀 앞으로의 예식만 센다). 숫자가 왜 줄었는지 여기 적혀야 안다.
     ⚠ 폰 경보는 취소로 안 울린다 — 경보는 취소·날짜와 무관한 수로 본다(20260830a) */
...
const FB_PER = 10;       // 한 페이지에 보여줄 응답 수 (대표 2026-09-15 «10개씩 페이지 네이션 해줘 30개는 너무 길어»)
