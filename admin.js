...
/* 작가에게 보낼 안내문 (대표 2026-08-31 «작가 안내문을 만들자»).
   링크는 작가마다 다르다 — 작가 관리에서 [링크 복사] 를 누르면 그 사람 것이 만들어진다.

   ⚠ 카카오톡에 붙여넣는 글이다. 줄을 짧게 끊는다 — 폰에서 줄이 길면 안 읽으신다.
   ⚠ 여기 적은 것이 실제 화면과 어긋나면 안 된다. 화면을 고치면 이 글도 같이 고친다.
   ⚠ 제일 중요한 것을 맨 위에 둔다. 아래로 갈수록 덜 급한 것이다 —
     끝까지 안 읽으셔도 홈 화면 추가와 알림 켜기는 하시게. */
function staffCalMsg(id) {
  const s = staffMap[id] || {};
  const url = location.origin + '/staff-calendar?s=' + id;
  const ch = STAFF_CHANNEL
    ? ['', '─────────', '',
       '카카오톡 채널 [' + STAFF_CHANNEL_NAME + '] 을 추가해두시면',
       '저희가 보낸 톡이 한곳에 모이고, 궁금한 것도 바로 물어보실 수 있습니다.',
       STAFF_CHANNEL]
    : [];
  return [
    '[온더브라이드] ' + (s.name || '') + ' 작가님, 캘린더가 새로워졌습니다.',
    '',
    '이제 예식이 배정되면 캘린더에 바로 뜨고 알림도 갑니다.',
    '따로 보내드리지 않아도 여기서 다 보실 수 있습니다.',
    '',
    url,
    '',
    '■ 두 가지만 부탁드립니다',
    '',
    '1. 홈 화면에 추가',
    '   캘린더 맨 아래 [📱 홈 화면에 추가] 를 눌러주세요.',
    '   카톡을 뒤지지 않고 바로 여실 수 있습니다.',
    '',
    '2. 알림 켜기',
    '   홈 화면에 생긴 앱을 열고 [🔔 알림 받기] 를 눌러주세요.',
    '',
    '   ※ 이 둘은 카카오톡 안에서는 안 됩니다.',
    '   위 링크를 누른 뒤 오른쪽 위 ⋮ 메뉴에서',
    '   [다른 브라우저로 열기] (사파리 · 크롬) 를 눌러주세요.',
    '',
    '■ 새로 생긴 [확인] 칸',
    '',
    '확인하실 일이 생기면 맨 위에 안내가 뜨고 숫자가 붙습니다.',
    '· 새 예식이 배정되었을 때',
    '· 예식 날짜·시간·장소가 바뀌었을 때',
    '· 연회장·폐백·2부 촬영이 붙거나 빠졌을 때',
    '· 예식이 취소되었을 때',
    '보시고 [확인했어요] 를 눌러주시면 됩니다.',
    '',
    '■ 캘린더에 적어주실 것',
    '',
    '· 촬영이 안 되는 날은 날짜를 눌러 [촬영불가]',
    '· 다른 촬영이 있는 날은 [다른촬영등록] 에 시간과 장소',
    '  저희 예식과 4시간 이상 벌어지면 저희 촬영도 가능한 날로 잡힙니다',
    '· 개인 일정은 [개인일정등록]',
    '  저에게는 「개인 일정」 이라고만 보입니다. 안심하고 쓰셔도 됩니다.',
    '  그날은 촬영 불가로 보고 배정에서 빼둡니다.',
    '',
    '스케줄은 틈틈이 채워주시면 배정이 훨씬 수월합니다.',
    '이미 배정된 예식은 불가로 바꿀 수 없습니다. 어려우시면 저에게 연락 주세요.',
    '',
    '■ 그 밖에',
    '',
    '· 배정된 예식의 날짜·시간·장소·신랑신부·상품·촬영 옵션을 보실 수 있습니다',
    '  (신랑신부 연락처는 예식 2주 전부터 보입니다)',
    '· [내 기록] 에서 지금까지 하신 촬영과 받으신 후기를 보실 수 있습니다',
    '· [설정] 에서 알림과 스케줄 받기를 켜고 끄실 수 있습니다',
    '· 자세한 사용법은 [안내] 칸에 다 적어두었습니다',
    '',
    '링크는 작가님 전용입니다. 다른 분께 전달하지 말아 주세요.',
  ].concat(ch).join('\n');
}
...
function renderStaff() {
  ...
  $('staffList').innerHTML = allStaff.map((s) => `
    <div class="staff-item${s.active ? '' : ' inactive'}" data-id="${s.id}">
      <!-- 폰에서 흩어지지 않게 세 묶음으로 나눈다 (대표 «모바일이 영 정리정돈이 안되네», 2026-08-28).
           넓은 화면에서는 display: contents 라 예전처럼 한 줄로 늘어선다 -->
      <div class="st-r1">
      ...
      <div class="st-r3">
        <!-- 캘린더 열기·안내문 복사 — 글자를 떼고 그림만 (대표 «이걸 이모티콘으로»)
